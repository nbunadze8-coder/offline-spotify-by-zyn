...
const UI = (() => {
  ...
  return {
    ...
    newPlaylist, addSongToPlaylist, removeSongFromPlaylist,
...
async function handleFileUpload(files) {
  ...
  for (const file of mp3Files) {
    ...
    try {
      const [tags, buffer] = await Promise.all([
        parseID3(file),
  ...
  progressText.textContent = `Done! Added ${processed} songs.`;
  setTimeout(() => progress.classList.add('hidden'), 2000);

  toast(`Added ${processed} song${processed !== 1 ? 's' : ''}`);
